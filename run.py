...
class run:
    ...
    def test(self, path):
        ...
        with tf.compat.v1.Session(config=self.config) as sess:
            ...
            ori_psnr = self.psnr(cropped, HR_image)
            bicubic_psnr =self.psnr(cropped, bicubic_image)

            print("PSNR of FSRCNN  upscaled image: {}".format(ori_psnr))
            print("PSNR of bicubic upscaled image: {}".format(self.psnr(bicubic_psnr)))
